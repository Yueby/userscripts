...
export class ItemEditAPI extends BaseAPI<ItemEditAPI> {
	...
	private loadTagElements(): void {
		...
		// combobox 的直接父级 div.w-full 作为 inputContainer
		const inputContainer = combobox?.closest(".w-full") as HTMLElement;
		...
		const observer = new MutationObserver((mutations) => {
			for (const mutation of mutations) {
				for (const node of Array.from(mutation.addedNodes)) {
					...
					// 情况1：整个列表容器被添加（初始化）
					if (
						this.isTargetListContainer(node) &&
					...
					// 情况2：单个列表项被添加（用户操作）
						...
						if (
							container &&
							this.isTargetListContainer(container) &&
	...
	private findDragHandle(itemElement: HTMLElement): HTMLButtonElement | null {
		...
		const firstButton = itemElement.querySelector<HTMLButtonElement>(
	 ...
	 * @returns 文件元素数组
	 */
	private parseFileListFromContainer(
	...
	findVariationEditButton(index: number): HTMLElement | null {
		...
		if (filesSection) {
			const buttons = Array.from(
				filesSection.querySelectorAll<HTMLButtonElement>("button"),
			);
			const editButton = buttons.find((btn) => {
				const text = btn.textContent?.trim() || "";
				return (
					text === "Edit" || (text.includes("Edit") && !text.includes("Delete"))
				);
			});
	 ...
	 */
	async openVariationEditPanel(
		index: number,
		timeout = 5000,
	): Promise<VariationEditPanel | null> {
	...
	getSelectedFileIds(panel: VariationEditPanel): string[] {
		return panel.files
			.filter((f: DownloadableFileElement) => f.checked)
			.map((f: DownloadableFileElement) => f.id);
	}
	...
	async setVariationFiles(
		...
		const currentSelectedIds =
			this.getVariationSelectedFileIds(variationIndex).sort();
	...
	moveVariation(fromIndex: number, toIndex: number): boolean {
		...
		if (
			fromIndex < 0 ||
			fromIndex >= variations.length ||
			toIndex < 0 ||
			toIndex >= variations.length ||
			fromIndex === toIndex
		) {
			return false;
		}

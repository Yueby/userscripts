/**
 * SVG 图标集合
 * 所有图标使用 Feather Icons 风格
 */

/**
 * 为 SVG 图标设置尺寸和 stroke-width
 * @param svg SVG 字符串
 * @param size 尺寸（宽高相等）
 * @param strokeWidth 可选的 stroke-width
 */
export const withSize = (svg: string, size: number, strokeWidth?: number): string => {
